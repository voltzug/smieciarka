...
export default function selectFlow(): void {
  ...
  try {
    switch (kind) {
      case 0:
        db.query(
          `SELECT id, login FROM core.users WHERE lower(login) LIKE $1 LIMIT 20`,
          `${prefix.toLowerCase()}%`,
        );
        break;
      case 1:
        db.query(
          `SELECT id, sn, title FROM core.items WHERE title ILIKE $1 LIMIT 20`,
          `${prefix}%`,
        );
        break;
      case 2:
      ...
      case 3: {
        ...
        db.query(
          `SELECT id, value, status FROM data.bids WHERE offer_id = $1`,
          offerId,
        );
        break;
      }
