...
export function registerItemOffer(
  ...
  return withTx(db, () => {
    ...
    if (items[0].creator_id !== creatorId) {
      throw new Error(`User ${creatorId} does not own item ${itemId}`);
    }
    ...
    if (existing.length > 0) {
      ...
      const inserted = db.query(
        ...
        price, description, itemId, creatorId
      )[0];
...
export function cancelItemOffer(db: Db, userId: number, offerId: number): void {
  withTx(db, () => {
    ...
    if (!rows.length) throw new Error(`Offer ${offerId} does not exist`);
    const offer = rows[0];

    if (offer.creator_id !== userId) throw new Error(`User ${userId} does not own offer ${offerId}`);
    if (offer.status !== 'ACTIVE' && offer.status !== 'RESERVED') {
      throw new Error(`Offer ${offerId} cannot be cancelled (status=${offer.status})`);
    }
    ...
    const { hash } = db.query(
      `SELECT encode(data._offer_hash($1, $2, $3, $4::money, $5, $6::timestamptz), 'hex') AS hash`,
      offerId, offer.item_id, userId, offer.price, offer.description, ts
    )[0];

    db.query(
      `SELECT audit.append_item_event($1, $2, $3, 'CANCEL_OFFER'::audit.e_item_event_type, decode($4, 'hex'))`,
      offer.ledger_head, offer.item_id, userId, hash
    );
  });
}
