    ...
    try {
      ...
      if (!rows.length) {
        throw new BusinessLogicError(
          `Bid ${bidId} not found or seller ${sellerId} is not item owner`,
        );
      }
      const r = rows[0];
      ...
      if (r.offer_status !== "ACTIVE" && r.offer_status !== "RESERVED") {
        throw new BusinessLogicError(
          `Offer is not in a winnable state (status=${r.offer_status})`,
        );
      }
      if (r.status !== "PENDING") {
        throw new BusinessLogicError(
          `Bid ${bidId} is not pending (status=${r.status})`,
        );
      }
