  ...
  withTx(db, () => {
    try {
      const items = db.query(
        `SELECT title, ledger_head FROM core.items WHERE id = $1 AND creator_id = $2 FOR UPDATE NOWAIT`,
        itemId,
        userId,
      );
      if (!items.length)
        throw new BusinessLogicError(
          `Item ${itemId} not found or not owned by user ${userId}`,
        );
...
export function changeItemDetails(
  db: Db,
  itemId: number,
  newTitle: string,
  userId: number,
  ...
  withTx(db, () => {
    try {
      const items = db.query(
        `SELECT sn, ledger_head FROM core.items WHERE id = $1 AND creator_id = $2 FOR UPDATE NOWAIT`,
        itemId,
        userId,
      );
      if (!items.length)
